...
/**
 * Load the local Notion page map from the cache file.
 *
 * Reads and parses MAP_FILE if it exists and returns the parsed object; returns an empty object when the file is absent.
 * @returns {Object} Mapping of "parentId/title" to Notion page IDs, or an empty object if no map file exists.
 */
function getMap() {
...
/**
 * Persist the page mapping to the local map file as formatted JSON.
 * @param {Object} map - Object mapping "parentId/title" keys to Notion page IDs.
 */
...
/**
 * Send an authenticated HTTP request to the Notion API.
 *
 * @param {string} endpoint - The Notion API path relative to `v1/` (e.g., `pages`, `blocks/{id}/children`).
 * @param {string} [method='GET'] - HTTP method to use.
 * @param {object|null} [body=null] - Request body which will be JSON-serialized when provided.
 * @returns {any} The parsed JSON response body from Notion.
 * @throws {Error} If the Notion API key is missing or if the response status is not OK; error message includes the HTTP status and response body.
 */
...
/**
 * Find a child Notion page by its exact title under the specified parent and return its page id.
 *
 * @param {string} title - The exact page title to match.
 * @param {string} parentId - The parent page id (may include dashes); dashes will be ignored when matching.
 * @returns {string|null} The matching Notion page id if found, `null` otherwise.
 */
...
/**
 * Ensures a Notion page with the given title exists under the specified parent, creating it if missing and caching the page ID locally.
 * @param {string} title - The page title to find or create.
 * @param {string} parentId - The Notion parent page ID under which to find or create the page.
 * @param {string} [content=""] - Optional initial content in Markdown to populate the page when it is created.
 * @returns {string} The Notion page ID of the existing or newly created page.
 */
 ...
 *
 * Supports top-level headings written as lines beginning with "# " (converted to `heading_1`)
 * and non-empty lines converted to `paragraph` blocks. Empty lines are ignored.
 *
 * @param {string} markdown - Markdown text to convert.
 * @returns {Array<Object>} An array of Notion block objects suitable for appending as page children.
 */
...
/**
 * Archive a Notion page and remove any local map entries that reference it.
 *
 * @param {string} pageId - The Notion page ID to archive. Any local mappings pointing to this page will be removed.
 */
...
/**
 * Append Markdown-derived blocks to a Notion page only if the page has no existing child blocks.
 *
 * If the target page already contains child blocks, the function leaves the page unchanged and returns.
 * Otherwise, it converts the provided Markdown into Notion block objects and appends them in batches
 * (up to 100 blocks per request) to the page's children.
 *
 * @param {string} pageId - Notion page ID to update.
 * @param {string} content - Markdown content to convert into Notion blocks and append.
 */
...
/**
 * Ensure a Notion page exists for a local Markdown file and update its content.
 *
 * @param {string} filePath - Path to the local Markdown file to sync.
 * @param {string} parentPageId - Notion page ID under which the file's page should reside.
 * @returns {string} The Notion page ID for the synced page.
 */
...
/**
 * Recursively syncs a local directory into Notion, creating or updating pages for subfolders and Markdown files under a specified parent page.
 *
 * Subdirectories (except hidden ones starting with '.') are ensured as child pages and traversed recursively. Markdown files (*.md) in the directory are synced as pages under the given parent.
 *
 * @param {string} dirPath - Filesystem path of the directory to sync.
 * @param {string} parentPageId - Notion page ID to use as the parent for created or updated pages.
 */
